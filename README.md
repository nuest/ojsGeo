...
A more comprehensive **feature booklet** with one screenshot per page covering every user-visible feature of the plugin (submission form, editorial workflow, article view, issue map, journal-wide map, admin settings, internationalisation) is generated by the Playwright script alongside the testData dump. Browse the descriptions, links to the related GitHub issues, and the URL each shot was taken at in [`screenshots/report.md`](screenshots/report.md) (rendered nicely on GitHub). To produce the matching timestamped PDF (`screenshots/geoMetadata-screenshots-YYYY-MM-DD-HHMMSS.pdf`) locally — useful when preparing release notes or sharing demo material — install [pandoc](https://pandoc.org/) plus a LaTeX engine (e.g. xelatex via `texlive-xetex`) and run:

```bash
cd ../headless                                  # sibling repo with the playwright workspace
node take-geometadata-screenshots.mjs           # produces screenshots/*.png + report.md + the timestamped PDF
```

The PNG / PDF / log artefacts are gitignored; only `screenshots/report.md` is checked in.
