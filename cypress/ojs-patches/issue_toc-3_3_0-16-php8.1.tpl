 ...
 *
 * Patched for OJS 3.3.0-16 (php8.1) to add the `Templates::Issue::TOC::Main`
 * hook that the geoMetadata plugin uses to render its per-issue map section.
 * Upstream `issue_toc.tpl` is identical between 3_3_0-11 and 3_3_0-16 — the
 * hook is still missing in stock OJS. See README.md "Configure Issue TOC".
 * Mounted into the cypress OJS container via docker-compose-mysql.yml.
 *
